...
Page({
  ...
  drawPointLine () {
    ...
    for(let i = 0, pLen = pointArr.length; i < pLen; i++){
      ...
      if(pointArr[i + 1]){
        if((switchBtn && balanceArr[i + 1] > balanceArr[i]) || (!switchBtn && (balanceArr[i] > 0 || balanceArr[i + 1] > 0))){
      ...
      context.fill()
      context.fillText((!switchBtn && balanceArr[i] > 0 ? '+' : '') + balanceArr[i], pointArr[i].x + 3, pointArr[i].y - 3)  // 点的含义，画字
      context.closePath()
    }
