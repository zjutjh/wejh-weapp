 ...
 * @returns {Array}
 */
function fixTimetable(classResult) {
  if (!classResult) {
    return [];
  }
    ...
    infoList.forEach((info) => {
      ...
      for (let i = 1; i <= 18; i++) {
        if (type === "odd" || type === "even") {
          if (type === "odd") {
            lesson["周"][i] =
              i >= info["开始周"] && i <= info["结束周"] && (i + 1) % 2 === 0;
          } else {
            lesson["周"][i] =
              i >= info["开始周"] && i <= info["结束周"] && i % 2 === 0;
          }
        } else {
      ...
      lessons[parseInt(info["星期"]) - 1][jie] &&
        lessons[parseInt(info["星期"]) - 1][jie].push(
...
function fixCardCost(cardData) {
  ...
  return {
    ...
    cost: records,
    text:
      records.map((item) => `<i style="${iStyle}">${item}</i>`).join(" + ") +
      " = " +
      `<i style="${iStyle}${totalStyle}">${total}</i>`,
  };
}
function fixTeacher(teacherData) {
...
function getTrueScore(scoreString) {
